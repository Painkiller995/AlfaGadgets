...
var LibPath =
  "file:///" +
  System.Gadget.path.replace(new RegExp("\\\\", "g"), "/") +
  "/SoundControl.dll";
...
function debugLog(str) {
  try {
    if (bDebug) {
      ...
      var debugLogFile = oFSO.OpenTextFile(
        System.Gadget.path + "\\debug.txt",
        iFlag
      );
...
function RegisterLibrary() {
  ...
  try {
    ...
    oShell.RegWrite(
      clsidRoot + "InprocServer32\\Assembly",
      LibName + ", Version=1.0.2588.9125, Culture=neutral, PublicKeyToken=null",
      "REG_SZ"
    );
    oShell.RegWrite(
      clsidRoot + "InprocServer32\\RuntimeVersion",
      "v4.0.30319",
      "REG_SZ"
    );
    oShell.RegWrite(clsidRoot + "InprocServer32\\CodeBase", LibPath, "REG_SZ");
    oShell.RegWrite(
      clsidRoot + "InprocServer32\\1.0.2588.9125\\Class",
      Classname,
      "REG_SZ"
    );
    oShell.RegWrite(
      clsidRoot + "InprocServer32\\1.0.2588.9125\\Assembly",
      LibName + ", Version=1.0.2588.9125, Culture=neutral, PublicKeyToken=null",
      "REG_SZ"
    );
    oShell.RegWrite(
      clsidRoot + "InprocServer32\\1.0.2588.9125\\RuntimeVersion",
      "v4.0.30319",
      "REG_SZ"
    );
    oShell.RegWrite(
      clsidRoot + "InprocServer32\\1.0.2588.9125\\CodeBase",
      LibPath,
      "REG_SZ"
    );
    ...
    oShell.RegWrite(
      clsidRoot +
        "ProgId\\Implemented Categories\\{62C8FE65-4EBB-45E7-B440-6E39B2CDBF29}\\",
      "",
      "REG_SZ"
    );
  } catch (err) {
...
function GetLibrary() {
  ...
  if (Lib == null) debugLog("Error creating ActiveX object");
  else return Lib;
}
